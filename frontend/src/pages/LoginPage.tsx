    ...
    <div className="min-h-screen bg-bg flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-4">
          <h2 className="text-lg font-semibold">{t('auth.prihlasenie')}</h2>
          ...
          <button type="submit" disabled={submitting} className="btn-primary w-full">{submitting ? t('auth.prihlasujem') : t('auth.prihlasit_sa')}</button>
          <p className="text-xs text-text-secondary text-center">
            {t('auth.nemas_ucet')} <Link to="/register" className="text-accent-primary hover:underline">{t('auth.register_link')}</Link>
          </p>
        </form>
        ...
        <button onClick={discordLogin} className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-white transition" style={{background: '#5865F2'}}>
          <DiscordIcon />
          {t('auth.discord_prihlasenie')}
        </button>
      </div>
    </div>
  )
