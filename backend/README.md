...
To enable Discord login, create an application at https://discord.com/developers/applications and set these environment variables before starting the backend:

| Variable | Description |
|---|---|
| `DISCORD_CLIENT_ID` | Discord application client ID |
| `DISCORD_CLIENT_SECRET` | Discord application client secret |
| `DISCORD_REDIRECT_URI` | (optional) Defaults to `http://localhost:8080/api/auth/discord/callback` |
| `DISCORD_FRONTEND_URL` | (optional) Defaults to `http://localhost:5173` |

In your Discord app's OAuth2 settings, add the redirect URI matching `DISCORD_REDIRECT_URI`.
The required scopes are `identify` and `email`.
